	...
	$.ajax({
		...
		'success':function(data){
			...
			for (var i = 0 ; i <=abc.length-1; i++) {
					...
					toAppend+="<div class='grid_1_of_4 images_1_of_4' ><a  onclick='productdetails("+abc[i].pk_int_product_id+")'><img src='<?php echo base_url(); ?>uploads/"+abc[i].vchr_product_image+"'></a><h2>"+abc[i].vchr_product_name+"</h2><div class='price-details'><div class='price-number' style='font-size:20px;'>Rs."+abc[i].int_selling_price+"</div><div class='add-cart'><button type='button' class='btn btn-danger' value='"+abc[i].int_quantity+"'  onclick='productpurch("+abc[i].pk_int_product_id+",\""+abc[i].int_selling_price+"\",\""+abc[i].int_quantity+"\")'>Buy Now</button></div></div></div></div>";
			};
			$('#pc').append(toAppend);
		}
});
...
{    
	...
	$.ajax({
		'type' :"POST",
		'url' :"http://api.baabtra.com/homeshop_services/index.php/welcome/showdetails",
		'datatype':"json",
		'data':{name:pk},
		'success':function(data){
			var abc= JSON.parse(data);
			...
			$('#de').load('preview1',{a:id,b:name,c:price,d:desc,e:img,f:quantity});
		}
	});
}
